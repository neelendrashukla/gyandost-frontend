...
import { fetchExamQuestions, saveExamResult, awardXpAndLogActivity } from "../api.js"; 
...
const ExamConfig = ({ config, setConfig, startExam, loading }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-card">
    ...
    <div className="space-y-4">
      <div>
        ...
        <select value={config.subject} onChange={e => setConfig({ ...config, subject: e.target.value })} className="w-full p-3 border border-gray-300 rounded-lg mt-1 focus:ring-2 focus:ring-brand-primary">
          {subjects.map(s => <option key={s} value={s}>{s}</option>)}
        </select>
      </div>
      <div>
        <label className="font-semibold text-gray-700">Chapter (optional)</label>
        <input type="text" value={config.chapter} onChange={e => setConfig({ ...config, chapter: e.target.value })} placeholder="Chapter name" className="w-full p-3 border border-gray-300 rounded-lg mt-1 focus:ring-2 focus:ring-brand-primary"/>
      </div>
      <div>
        <label className="font-semibold text-gray-700">Number of Questions</label>
        <select value={config.count} onChange={e => setConfig({ ...config, count: parseInt(e.target.value, 10) })} className="w-full p-3 border border-gray-300 rounded-lg mt-1 focus:ring-2 focus:ring-brand-primary">
      ...
      <motion.button onClick={startExam} disabled={loading} whileHover={{ scale: 1.02 }} className="w-full bg-brand-primary text-white font-bold py-3 px-4 rounded-lg text-xl hover:bg-blue-700 disabled:opacity-50">
...
// --- Exam Player ---
const ExamPlayer = React.memo(({ questions, config, timeLeft, formatTime, handleAnswer, answers, currentQ, setCurrentQ, finishExam }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 bg-white rounded-lg shadow-lg">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-gray-700">{config.subject}: {config.chapter || 'Full Test'}</h2>
        <div className="text-xl font-bold bg-red-500 text-white px-3 py-1 rounded-full">{formatTime(timeLeft)}</div>
      </div>

      <div className="w-full bg-gray-200 rounded-full h-2 mb-6 overflow-hidden">
        <motion.div className="bg-blue-500 h-2 rounded-full" initial={{ width: 0 }} animate={{ width: `${(currentQ / questions.length) * 100}%` }} transition={{ duration: 0.4 }}/>
      </div>

      <AnimatePresence mode="wait">
        <motion.h3 key={currentQ} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="text-2xl font-semibold mb-6 min-h-[6rem] text-gray-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 justify-items-center">
        {q.options.map(opt => {
          ...
          return (
            <motion.button key={opt} onClick={() => onOptionClick(currentQ, opt)} whileHover={{ scale: 0.95 }} whileTap={{ scale: 0.85 }} className={`relative max-w-md w-full flex justify-between items-center p-4 rounded-lg border-2 text-left text-lg transition-all duration-200 shadow-sm ${isSelected ? 'bg-gradient-to-r from-blue-500 to-indigo-500 border-blue-600 text-white shadow-md scale-105' : 'bg-gray-50 border-gray-200 text-gray-800 hover:bg-blue-50 hover:border-blue-300'}`}>
              <div className="flex items-center justify-center gap-2 w-full">
                <span className="font-semibold text-center">{opt}</span>
                {isSelected && <motion.span initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="flex items-center gap-1 text-sm bg-green-500 text-white px-2 py-1 rounded-full shadow-md"><span>👈</span><span className="hidden sm:inline">Selected</span></motion.span>}
              </div>
            </motion.button>
          );
        })}
      ...
      <div className="flex justify-end mt-8">
        {currentQ < questions.length - 1 ? (
          <motion.button onClick={() => setCurrentQ(q => q + 1)} disabled={!answers[currentQ]} whileHover={{ scale: 1.05 }} className="px-6 py-2 bg-brand-primary text-white rounded-lg disabled:opacity-50">Next →</motion.button>
        ) : (
          <motion.button onClick={finishExam} disabled={!answers[currentQ]} whileHover={{ scale: 1.05 }} className="px-6 py-2 bg-green-500 text-white rounded-lg disabled:opacity-50">Finish Exam</motion.button>
        )}
      </div>
    </motion.div>
  );
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-6 text-left space-y-4 overflow-hidden">
            {questions.map((q, index) => (
              <div key={index} className={`p-4 rounded-lg ${answers[index] === q.correct_answer ? 'bg-green-50 border-green-500' : 'bg-red-50 border-red-500'}`}>
                ...
                {q.explanation && <div className="mt-2 p-2 bg-yellow-50 rounded-lg"><p className="text-sm text-yellow-800"><span className="font-bold">Explanation:</span> {q.explanation}</p></div>}
              </div>
            ))}
          </motion.div>
        )}
...
export default function ExamMode({ session }) {
  ...
  const startExam = async () => {
    ...
    try {
      ...
      if (data.questions && data.questions.length > 0) {
        ...
        const duration = config.count === 10 ? 7*60 : config.count === 20 ? 15*60 : 40*60;
  ...
  const finishExam = async () => {
    let finalScore = 0;
    questions.forEach((q, index) => { if (answers[index] === q.correct_answer) finalScore++; });
    setScore(finalScore);
    winSound.play();
    setView('result');
    ...
    if (session.user) {
      try {
        ...
        updateProfile();
      } catch { toast.error("Aapka result save nahi ho paaya."); }
    }
  };
